...
SQL Server 전체적인 상황을 대략 모니터링 할수 있는 카운터들
...
확장 이벤트 사용 하여 문제 쿼리를 식별해야 한다. 인덱스가 누락되거나 요청 된 행이 너무 많은 경우 또는 T-SQL 형식이 잘못된 쿼리는 테이블 이나 인덱스의 풀 스캔을 유발한다. 이렇게 되면 과도한 CPU 타임과 과도한 논리적 읽기가 동시에 발생하기 때문에 문제가 된다.  

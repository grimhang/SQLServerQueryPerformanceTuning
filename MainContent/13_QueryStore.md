...
쿼리 저장소는 세 가지 주요한 기능.  
    1. 쿼리의 성능수치와 실행 계획 등을 데이터베이스의 별도 저장공간에 영구적으로 저장하며 여러 유연한 방식으로 가져와 쉽게 성능 정보를 확인.  
...
이제 우리는 성능이 좋지 않은 쿼리를 손쉽게 식별하는 기능을 얻었다.  

쿼리 저장소의 기능은 매우 훌륭하지만 대부분의 사람들이 기존에 익숙하게 사용하고 있던 도구들을 완전 대체하지는 않는다. 게다가 확장 이벤트만큼 세분화되지도 않고 계획 캐시를 즉시 쿼리할 수도 없다.  

즉, 쿼리 저장소는 다음 기능의 추가로 확인할 수 있다.
    1. 값에 대한 표준 편차와 같은 추가 정보 
    2. 캐시에서 제거되거나 교체된 실행 계획을 포함하여 모든 실행 계획을 보유함으로써 이러한 두 가지 방법에 추가.
    3. 또한 쿼리 저장소는 매개 변수 또는 기타 동작과 관련된 문제뿐만 아니라
    4. Microsoft의 업그레이드로 인한 계획 회귀에도 도움이 되는 매우 간단한 계획 적용을 수행하는 기능이 추가되었다.  
이 모든 것이 결합되어 쿼리 저장소를 쿼리 튜닝 도구 키트에 매우 유용한 추가 기능으로 만듭니다. 

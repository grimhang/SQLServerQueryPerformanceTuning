...
이제 우리는 성능이 좋지 않은 쿼리를 손쉽게 식별하는 기능을 얻었다. 쿼리 저장소의 기능은 매우 훌륭하지만 대부분의 사람들이 이미 편안하게 사용하고 있는 도구를 완전히 대체하지는 않는다. 또한 확장 이벤트만큼 세분화되지도 않고 계획 캐시를 쿼리하는 즉시성도 없다. 즉, 쿼리 저장소는 값에 대한 표준 편차와 같은 추가 정보를 포함하고 캐시에서 제거되거나 교체된 실행 계획을 포함하여 모든 실행 계획을 보유함으로써 이러한 두 가지 방법에 추가됩니다. 또한 쿼리 저장소는 매개 변수 또는 기타 동작과 관련된 문제뿐만 아니라 Microsoft의 업그레이드로 인한 계획 회귀에도 도움이 되는 매우 간단한 계획 적용을 수행하는 기능을 추가합니다. 이 모든 것이 결합되어 쿼리 저장소를 쿼리 튜닝 도구 키트에 매우 유용한 추가 기능으로 만듭니다. 
...
RAID 5  
    RAID 5는 여러 경우에 좋은 옵션이다. RAID 0의 구조를 기반으로 단지 디스크 하나만 추가하여 효율적인 내결함성을 제공한다. 
    5개의 디스크가 있다면 다음과 같이 데이터를 저장하는 구조이다
    ...
    이를 메모리의 더티페이지라고 부르며 데이터 파일에는 체크포인트라는 내부 배치 작업에 의해 의해서 주기적 비동기 작업으로 하드디스크에
    기록된다.
...
**RAID 6**  
    ...
    RAID 5가 패리티를 1개 디스크에 쓰는데 비해 6는 2개 디스크에 쓴다. 내결함성은 5보다 훨씬 증가하는 것은 당연하지만 단점도 2배가 된다.
    5가 쓰기가 약점인데 6는 더욱더 쓰기에 취약하다. Sql Server에는 맞지 않다.

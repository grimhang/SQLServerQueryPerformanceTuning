...
* Checkpoint Pages/Sec  
    SQL Server 엔진은 성능상의 이유로 변경 내용이 있을 때마다 메모리(버퍼캐시)에서 데이터베이스 페이지를 수정하며 이러한페이지를 바로 디스크에 기록하지는 않는다. 메모리상에서만 변경되고 디스크에 기록되기 전의 데이터베이스 페이지를 "더티 페이지" 라고 한다.
    ...
    이 수치가 높으면 시스템에 쓰기 작업이 많고 I/O 문제를 발생할 가능성이 있다.
    ```sql
    SELECT *
    ...
    */
    ```
    "recovery intervals" SQL 서버 범위의 체크포인트 주기를 결정하는 값이다. 기본값은 0이고 60초 단위로 체크포인트를 수행한다는 뜻이다.  
    ...
    */
    ```    
    "target_recovery_in_seconds" 값을 바꾸면 데이터베이스 단위로 체크포인트 주기를 결정할 수 있게 된다. 기본값은 0이고 이 때는 서버 구성을 따른다는 의미이다.  만약 이 값이 0이 아니라면 서버 설정은 무시하고 데이터베이스 쪽 설정이 우선된다.  
    ...
    단 "Checkpoint Pages/Sec" 가 30을 넘기 때문에 메모리상의 더티 페이지가 많이 발생함을 알 수 있고 메모리와 디스크I/O의 성능 이슈가 발생할 가능성이 높다.  
    잘 안보이지만 파란색 선이 디스크의 평균 write 카운터이다. 정확히 1분마다 체크포인트 발생시만 I/O가 치솟는 것을 알 수 있다.  
    이런 경우 우리는 1분마다 과도한 I/O를 발생하는 것을 피하며 I/O를 고르게 분산시킬 목적으로 60초보다 더 짧은 체크포인트 값을 지정해야 한다.  

...
메모리에 더티 페이지가 있을 때 평소대로라면 체크포인트에 의해 기본값 60초 주기로 디스크에 기록을 하게 된다.  
체크포인트 작업으로 디스크에 기록이 완료된 예전 더티 페이지는 이제부터는 클린 페이지라고 불리게 되며 다른 버퍼 캐시와 같이 캐시 데이터의 역할을 하게 된다. 하지만 이 60초도 못 기다릴 만큼 메모리 공간이 부족하게 되면 이 때 더티 페이지의 공간도 같이 비워줘야 한다. Lazy writer 프로세스가 더티 페이지를 디스크로 기록을 하게 되고 기존 더티 페이지가 있던 버퍼캐시 공간 확보.

    메모리의 버퍼 캐시에 더티페이지 존재
        --> 메모리가 모자라 체크포인트 주기(60초) 기다릴 수 없을 만큼 공간이 모자르게 되고 빨리 방 비워달라고 재촉당함
            --> 클린 페이지들 공간을 비워지게 되며 이 때 더티 페이지는 lazy writer프로세스에 의해 디스크에 쓰여지고 공간 비워줌.
                --> 그렇게 확보된 버퍼 캐시 공간은 다른 sql server 세션들이 사용
...
```
    a. 체크포인트 : 메모리 버퍼 캐시에 있는 더티페이지가 디스크에 기록된 후에도 클린 페이지로 전환 되며 계속 버퍼 캐시에 존재

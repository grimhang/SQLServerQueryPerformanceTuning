...
Checkpoint Pages/Sec 카운터는 체크포인트로 인해 디스크로 이동하는 페이지들의 초당 숫자.
대부분의 경우 30보다 낮아야 한다. 이보다 높은 수치는 메모리 버퍼에 더티 페이지로 표시된 페이지들이 많다는 것을 의미한다.
더티 페이지란 하드디스크에 기록되지 않고 메모리 버퍼에서만 수정된 페이지들을 말한다. 일단 메모리에서만 수정되면 더티 페이지로 표시되고 다음 체크포인트때 디스크에 기록된다.
이 수치가 높으면 시스템에 쓰기 작업이 많고 I/O 문제를 발생할 가능성이 있다.
만약 새로운 간접 체크 포인트를 활용한다면 체크포인트 발생시 "recovery interval" 을 줄일 수 있게 조절할 있다. 그러면 다른 수치를 볼지도 모른다.
데이터베이스별로 의 간접 체크포인트 구성하여 이 값을 조절 할 수도 있다.

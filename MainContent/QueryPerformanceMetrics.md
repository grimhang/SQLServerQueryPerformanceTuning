...
이 모든 조건들은 캡처되는 이벤트에 필터를 거는 것처럼 작동하며 나중에 수집된 결과물의 크기를 줄여주고 또한 분석할때도 보다 명확한 범위의 데이터만 하기 때문에 많은 시간을 줄여준다. 아래 표는 성능 분석을 위해 일반적으로 사용되는 필터 조건드ㄴㄹ이다.
    ...
    Events          Filter Criteria Example    UseCategory  
    --------------  -------------------------  --------------------  
    sqlserver.username      = user_apple            단일 사용자나 로그인으로 제한
    sqlserver.database_id   = 7                sp_helpdb에 나오는 데이터베이스id로 단일 데이터베이스 제한
    duration        >= 2000                      많이 사용. 2000마이크로초(0.002) 이상 소요되는 것만 캡처되게 제한. 나는 보통 3000000(3초)처럼 초를 지정함.
    ...
    일반 비교연산자(>=)와 greater_than_equal_unit64가 따로 있는데 하나는 32bit int이고 하나는 부호없는 64비트 int인듯하여 본인은 항상 unit64를 쓴다. (혹시 내가 잘못 알고 있는거면 알려주세요).
...
간단하게 GUI를 통해 확장 이벤트 세션을 만들수 있지만 아쉽게도 스케일할수 있는 방법은 아니다. 다중 서버에서 동일한 종류의 확장 이벤트 세션을 만들려면 일일이 각각 접속해서 수행해야 한다. 그렇기 때문에 t-sql 명령문을 통해 수행하면 보다 수월하다.
    ...
    - 생성된 세션 시작
        ...
        ```
    - 확장 이벤트 자동화
    SQL Agent 를 통해 이런 작업들을 자동화 할수 있거나 또는 sqlcmd.exe를 이용할 수도 있다. 어찌됐든 가장 마지막 작업은 세션을 ㅣㅅ작하는 것이다. 중지할때도 위 스크립트에서 START를 STOP으로 바꾸면 된다.
...
GO
```
    - CREATE EVENT SESSION : 세션 만들기
    - ADD EVENT : rpc_completed (주로 sp들)이벤트 추가
    - SET collect_statement : statement는 rpc_completed의 옵션 이벤트 필드인데 1인것 보니 선택됨.
...
세션이 시작되면 서버단에서 운영되기 때문에 SSMS가 필요 없다. 다음 DMV로 확인 가능.

    - sys.dm_xe_sessions  : 활성화된 확장 이벤트 세션들
...
```sql
    ...
    STATE = STOP;
```    
    세션 중지하면 sys.dm_xe_sessions에서 사라진다.
...
- ### **인과 관계 추적**
GUI나 T-SQL 명령문에서 세션을 정의하는 것은 간단하다. 하지만 보다 자세한 분석을 위해서는 단일 배치 명령문이나 단일 SP가 아닐 것이다. 궁극적으로는 프로시저나 배치에 들어있는 여러가지 sql들까지 같이 보기를 원한다. statement 레벨의 재컴파일, 대기, 정렬상황은 각각의 저장프로시저나 스테이트면트에 함께 있다. 그러므르 인과관계 추적이 필요한다.

GUI에서는 "인과 관계 추적" t-sql에서는 TRACK_CAUSALITY 를 체크하면 된다.

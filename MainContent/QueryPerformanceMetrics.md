...
# Query 성능 수집
SQL Server 성능 저하의 대부분은 데이터베이스 응용 프로그램이 과도한 워크로드를 발생시키기 때문이다. 그러기 때문에 과도한 워크로드를 발생시키는 SQL 쿼리를 식별하는 것이 문제를 해결하는 가장 첫 걸음이 된다.
이번 장에서도 쿼리 식별을 위한 **확장 이벤트**와 **여러가지 SSMS(매니지먼트 스튜디오) 툴**을 살펴 보게 된다.
...
확장이벤트는 SQL Server 2008에서 처음 소개되었는데 GUI는 없었고 세팅하기도 매우 복잡했다. 그렇기에 성능 측정치를 캡처하기 위한 도구로는 거의 사용되지 않았다. 2012가 되어서야 확장이벤트를 위한 편리한 GUI가 소개되었으며 가볍기 때문이기도 하여 성능데이터를 수집하기 위한 첫번째 옵션이 되었다. 이전에 프로파일러와 같은 툴로 쉽게 수집할 수 있었던 추적 이벤트 는 점점 추천하지 않게 되었으며 개발도 더이상 진행되지 않는다. 몇년동안 어떤 추적 이벤트 기능도 추가되지 않았다. 프로파일러는 운영서버에서 적절하지 않게 사용될 경우 성능상 많은 오버헤드가 발생할 수 있다. 결과적으로 이 책의 예제들은 첫번째로는 확장 이벤트를, 두번째로는 쿼리 저장소를 사용할 것이다.
...
SQL Server 인스턴스에서 발생하는 활동들을 캡처하기 위해 확장 이벤트를 사용할 수 있다. 설정은 그래픽UI나 쿼리 명령문를 이용해 할 수 있다. 확장 이벤트 세션을 만드는 가장 효과적인 방법은 T-SQL 명령어를 이용하는 것이다. 그러나 언제든 그렇든 처음 접할때는 GUI를 통해 세션에 대해 배워나가는 것도 좋다.
...
- ### a. 확장 이벤트 세션 만들기
    ...
    마법사를 이용해 새 세션을 만들수도 있지만 마법사 GUI는 사용이 쉽다는 장점이 있는 반면 설정할수 없는 옵션들도 있어서 여기서는 일반 "새 세션"을 통해 시작해 보자.
    ...
    - 첫번째 :  새 세션의 일반 옵션
        ...
        이 화면에서 세션이름을 지정해 준다.  
        나중에 세션의 기능을 명확하게 알 수 있게 의미있는 짧은 이름을 지정하는걸 권장한다.  
        또한 프로파일러처럼 미리 설정된 템플릿을 이용하면 최소의 노력으로 세션을 만들 수도 있다.          
        템플릿의 "쿼리 실행" 카테고리안에는 쿼리 튜닝과 연관된 5개가 존재한다.
        ...
        ```note
        세상에 위험이 없고 공짜인 일은 없다. 확장이벤트는 예전 프로파일러나 추적이벤트보다 시스템 정보를 수집하는 더 효과적인 메카니즘이긴 하나 비용이나 위험이 전혀 없을 수는 없다. 이 장의 뒤에서 좀더 자세히 설명하겠지만 우리가 정의하는 이벤트나 몇몇 전역 필드들에 따라 확장 이벤트 운영에 영향을 줄수 있다. 그렇기 때문에 운영 시스템에 네거티브 영향을 주지 않게 하기 위해 충분히 신중하게 사전 테스트한 후 적용하자.  
        다른 장에서 설명할 쿼리 저장소는 2016부터 지원하는데, 시스템에 보다 적은 영향을 주면서 DMO들을 통해 많은 정보를 제공해 주는 좋은 친구이다. 이런 대체 방법은 몇가지 상황에서 유용할 할수 있다.
        ```

        세션 만들기의 첫번째 화면에서 이름을 적는 이외에 몇가지 다른 옵션들이 있다. 서버 시작할때 자동으로 시작되도록 결정할 수도 있고 생성과 동시에 시작하고 라이브 데이터 감시를 바로 할수도 있다. 마지막으로 이벤트 관계 추적을 설정할 수도 있다. 이에 대한 자세한 설명은 뒤에서 한다.  
        오래동안 성능 항목들을 수집하면 할수록 저장되는 수집 데이터 크기도 증가한다. 
    ...
    - 두번째 : 이벤트 추가  
        ...
        여기에는 이벤트 대상, 이벤트 패키지, 이벤트 세션등으로 구성되어 있는 이벤트 아키텍처가 존재한다. 하지만 GUI사용은 그런 세부사항에 대해 걱정하지 않게 해준다. 뒷부분에서 세션을 스크립팅하는 방법을 보여줄때 일부 아키텍처 부분들도 같이 설명한다.
...
확장 이벤트는 프로그래밍적인 방법으로 정보를 수집하는 방법에서는 기존 프로파일러와 추적 이벤트와 비교해서 큰 오버헤드를 제거한 게임 체인저이다. 오버헤드를 줄이기 위해 수집하려는 이벤트나 필드의 개수를 제하하는 등 기존에는 걱정해야 하는 것을 줄였다. 그러나 이전에도 언급했듯 기존에 비해서는 매우 적어졌지만 여전한 오버헤드가 있을 수 밖에 없다. 다음과 같은 주의해야 할 점이 있다

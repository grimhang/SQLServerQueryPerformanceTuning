...
# Baseline 생성
나중에 비교할 기준이 될 수 있도록 주기적으로 성능 데이터를 수집하고 또한 이를 자동화 하는 방법도 같이 설명한다.  
성능모니터에서 로그파일로 저장하는 기능을 예전에는 "카운터 로그" 라고 불렀지만 지금은 "데이터 수집기 집합"이라고 한다. 이 책에서는 카운터 로그라는 명칭으로 설명한다.  

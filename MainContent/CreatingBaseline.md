...
- ### b. 성능카운터 리스트를 사용한 카운터 로그 만들기
    ...
    ```tip
    이진 파일인 .blg로 저장하면 파일을 더블클릭하는 것만으로 독립 실행형 성능 모니터 화면이 열려 바로 그래픽 UI로 분석할 수 있다.
    하지만 파일크기가 csv보다 몇배나 크기 때문에 실서버에서 측정하고 테스트서버로 옮기는 등의 절차때문에 권장하지 않는다. 대부분 csv로 저장.  나중에 blg를 csv로 변환할 수 있다.
    ```
